...
def get_strong_outflow_alerts(minutes=30, wallets=None, chains=None):
    ...
    for label, data in per_wallet.items():
        for ckey, info in data.items():
            ...
            if sym in STABLE_LARGE:
                continue
            st = token_stats.setdefault(ckey, {
                "contract": info.get("contract", ""),
                "symbol": info.get("symbol", "???"),
                "name": info.get("name", ""),
                "chain": info.get("chain", ""),
            ...
            st["amount"] += info["amount"]
            st["count"] += info["count"]
            st["score"] += info.get("score", info["amount"])
            st["wallets"].add(label)

    alerts = []
    for ckey, st in token_stats.items():
        n_wallets = len(st["wallets"])
        if n_wallets < MIN_WALLETS_FOR_ALERT:
            continue
        if st["score"] < MIN_SCORE_FOR_ALERT:
            continue
        if st["count"] < MIN_TRANSFERS:
            continue
